...
The vulnerability lies in one of the media engine commands that is responsible for color space conversion from YCbCr to RGBA. The command copies a row chosen by the user into a framebuffer, however doesn't sanitize check the row number.
...
I reverse engineered their `malloc()/free()` implementation and soon I designed a different method to exploit the heap overflow. Our goal is namely to apply an unlink attack.
...
After the overflow, the subroutine will quickly notice that the data that it received is actually wrong and bail out. On abort, the 0x800 bytes chunk will first be freed, then the 0x1000 (now zero) bytes chunk will be freed.
...
Nothing interesting happens here, but since these two chunks are still adjacent, we must merge once again:
